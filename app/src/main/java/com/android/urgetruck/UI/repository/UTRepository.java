...
public class UTRepository {
    ...
    public MutableLiveData<ExitClearanceInvoicingResponse> getExitClearanceInvoiceMutableLiveData
            (String baseUrl, int requestID, String vrn, String rfid) {
        this.baseUrl = baseUrl;
        ApiInterface apiService = APiClient.getClient(baseUrl).create(ApiInterface.class);
            ...
            public void onResponse(Call<ExitClearanceInvoicingResponse> call, Response<ExitClearanceInvoicingResponse> response) {
                if (response.isSuccessful()) {
                    ...
                    try {
                        ...
                        Toast.makeText(application.getApplicationContext(), "Error processing error response", Toast.LENGTH_SHORT).show();
                    }
                } else {
                    // Handle other error cases
                    Toast.makeText(application.getApplicationContext(), "Unexpected error occurred", Toast.LENGTH_SHORT).show();
                }
            }
    ...
    public MutableLiveData<ExitInvoiceUpdateListResponse> updateExitClearanceInvoiceMutableLiveData(String baseUrl, ExitClearanceInvoicingResponse exitClearanceInvoicingResponse) {
        this.baseUrl = baseUrl;
        ApiInterface apiService = APiClient.getClient(baseUrl).create(ApiInterface.class);
